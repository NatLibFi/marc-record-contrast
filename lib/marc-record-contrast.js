...
(function(root, factory) {
	...
	if (typeof define === 'function' && define.amd) {
		...
		root.MarcRecordContrast = factory(root.Q);
	}
}(this, function(Q) {
	"use strict";
	...
	function PreferredRecordSelector() {
		...
		this.normalizers = {
			reprintNormalizer: function(currentValue, otherValue) {

				if (otherValue.notesOnReprints.some(contains(currentValue.year))) {
					return 0;
				}
				if (currentValue.notesOnReprints.some(contains(otherValue.year))) {
					return 1;
				}
		...
		*/
		...
		this.extractReprintInfo = function(record) {
			var year = this.extractPublicationYear(record);
